...
namespace musicTherapy1
{
	public class Sessions : System.Windows.Forms.Form
	{
		...
		public Sessions(/*Size size,*/ int numOfPatiants, int sessionDuration, int timeIntervalBetweenBars)
		{
			...
			LableArray  = new Label[document[docNumber].numOfPatiants,document[docNumber].sessionDuration*2];
...
*/
		...
		{				
			...
			for ( int panelCounter  = 0;panelCounter <document[docNumber].numOfPanelsInSession;panelCounter++)
			{
				...
				CurlyBracesArray[panelCounter]=new PictureBox();
				try
				{
					// Retrieve the image.
					//Bitmap  image1 = new Bitmap(@"d:\Documents and Settings\Loki\" 
					//	+ @"My Documents\Visual Studio Projects\musicTherapy1.4.2\musicTherapy1.3\curlyBracats3.png", true);
					//Bitmap  image1 = new Bitmap(@"c:\curlyBracats3.png", true);
					Bitmap  image1 = new Bitmap(((MainForm)this.MdiParent).MainDirectory+"\\curlyBracats3.png");
					CurlyBracesArray[panelCounter].Image= image1;
				}
				...
				for (x = 0; x < document[docNumber].numOfPatiants; x++)
				{
					for (y = 0; y < numOfButtonsInRow; y++)
					{
						...
						LableArray[x, yy].Font=new System.Drawing.Font("David",12);
		...
		private void NameButtonClick(object sender, System.EventArgs e)
		{
			...
			if (nameDialogBox.ShowDialog() == DialogResult.OK)
			{
				...
				for (int panelCounter = 0 ;panelCounter<document[docNumber].numOfPanelsInSession;panelCounter++)
				{
					...
					sessionToolTip.SetToolTip(NameButtonArray[panelCounter,btn_y],"Additional information:\n " + this.document[docNumber].timeLine[btn_y].additionalInfoAboutPatiant);
				}
		...
		public void refeshVideoPointer(int position)
		{
			...
			/*int objectivePosition=((position/60)*2)+CurrentVidoeStartPoint;
			int panelNumber=(((position/60)*2)+CurrentVidoeStartPoint)/this.document[docNumber].numOfButtonsInTimeLine;
			int minutesInPanel=(((position/60)*2)+CurrentVidoeStartPoint)-((panelNumber)*this.document[docNumber].numOfButtonsInTimeLine);
			int Point=videoPanel[panelNumber].Size.Width/this.document[docNumber].numOfButtonsInTimeLine*minutesInPanel;
			*/
		...
		private void ButtonClick(object sender, System.EventArgs e)
		{
			...
			else if(this.RemoveInstrumentButtonPushed)
			{
				...
				//isWaitingForinstrumentEndSectionClick=false;
				//update all the buttons in the instrument section
				int startPoint=FindStartPointOfInstrument(b.y,b.x,this.document[docNumber].timeLine[b.y].MinutesArray[b.x].instrumetnInfo/*this.document[docNumber].timeLine[b.y].MinutesArray[b.x].instrumentCategorry,this.document[docNumber].timeLine[b.y].MinutesArray[b.x].instrumentSubCategory,this.document[docNumber].timeLine[b.y].MinutesArray[b.x].instrumentName*/ );
				int endPoint = FindEndPointOfInstrument(b.y,b.x,this.document[docNumber].timeLine[b.y].MinutesArray[b.x].instrumetnInfo/*this.document[docNumber].timeLine[b.y].MinutesArray[b.x].instrumentCategorry,this.document[docNumber].timeLine[b.y].MinutesArray[b.x].instrumentSubCategory,this.document[docNumber].timeLine[b.y].MinutesArray[b.x].instrumentName */);
		...
		public void refreshRichTextBoxes()
		{
			...
			for (int discCounter=0;discCounter<this.document[docNumber].RichTextList.Count;discCounter++)
			{
				...
				rich.Size =new Size(richTextBox.Text.Length*7, 25/*richTextBox.richTextBox.Size.Height*/); 
		...
		public void refreshDiscPanels()
		{
			...
			for (int discCounter=0;discCounter<this.document[docNumber].DiscList.Count;discCounter++)
			{
				...
				discPanel.Location = calculateDiscPanelLocation(0, disc.startPoint);//calculateSingScreamLoaction(disc.startPoint,0,40);
				discPanel.Size = calculateDiscPanelSize(disc.endPoint-disc.startPoint);//calculateSingScreamSize(disc.endPoint-disc.startPoint+1,40);
		...
		{
			int minuteCounter;
			for (minuteCounter=minute;minuteCounter<this.document[docNumber].numOfButtonsInTimeLine*2;minuteCounter++)
			{
				if (this.document[docNumber].timeLine[patiant].MinutesArray[minuteCounter].instrumetnInfo!=instrumentInfo)
					break;
			}
		...
		public void deleteTextTopicsFromButtons(string topicAndSubTopic)
		{
			for (int x=0;x<this.document[docNumber].numOfPatiants;x++)
			{
				for (int y=0;y<this.document[docNumber].sessionDuration;y++)
				{
		...
		private Point setTimeLinePanelLocation(int panelNumber)
		{
			...
			int x,y;
			if (document[docNumber].numOfPatiants>6)
			{
				y= ((timeLineSectionHeight - (document[docNumber].percentOfPanelAreaOfHoleWindow*timeLineSectionHeight/100)));
			}
			else
			{
				y= ((timeLineSectionHeight - (80*timeLineSectionHeight*document[docNumber].numOfPanelsInSession/100/document[docNumber].numOfPanelsInSession))/(document[docNumber].numOfPanelsInSession+1))*panelNumber+
					((80*timeLineSectionHeight/100/document[docNumber].numOfPanelsInSession )*(panelNumber-1));
			}
		...
		private Size setTimeLinePanelSize(int panelNumber)
		{
			...
			if (document[docNumber].numOfPatiants>6)
			{
				...
				y=(80* timeLineSectionHeight/100);
			}
			else
			{
				x=(document[docNumber].percentOfPanelAreaOfHoleWindow * timeLineSectionWidth /100);
				y=(80* timeLineSectionHeight/100/document[docNumber].numOfPanelsInSession);
			}
			return new Size(x,y);
		}
		private APanel setTimeLinePanel(int panelNumber)
		{
			...
			TimeLinePanel.Location = setTimeLinePanelLocation(panelNumber);//new Point(y,x);
			TimeLinePanel.Size = setTimeLinePanelSize(panelNumber); //new Size(x,y);
		...
		private Size calculateInstrumentPictureSize(Panel timeLinePanel)
		{
			...
			{
				//lableSize.Width=timeLinePanel.Size.Width/document[docNumber].numOfButtonsInTimeLine*90/100*2;//  double of the button	
				lableSize.Height=timeLinePanel.Size.Height/(document[docNumber].numOfPatiants+1)*90/100;
				lableSize.Width=(int)((double)lableSize.Height*1.4);
			}
		...
		private Point calculateCurlyBracesLocation(Panel timeLinePanel)
		{
			...
			CurlyBracesLication.Y= ((timeLinePanel.Size.Height-(document[docNumber].numOfPatiants*document[docNumber].widthOfTimeLineButton))/(document[docNumber].numOfPatiants+1) * (0+1)) +(document[docNumber].widthOfTimeLineButton*((0+1)-1)); 
			return CurlyBracesLication;
		}
		private Size calculateCurlyBracesSize(Panel timeLinePanel)
		{
			Size buttonStartSize=new Size();
			buttonStartSize.Width=timeLinePanel.Size.Width*document[docNumber].percentOfSpaceBeforeButtonsInTimeLine/100*(100-document[docNumber].percentOfSizeOfNameButtonOfSizeOfSpaceBeforeButtonInTimeLine-document[docNumber].percentOfSizeOfSpaceBetweenNameButtonsAndCurlyBraces)/100;
			buttonStartSize.Height=((timeLinePanel.Size.Height-(document[docNumber].numOfPatiants*document[docNumber].widthOfTimeLineButton))/(document[docNumber].numOfPatiants+1) * (document[docNumber].numOfPatiants-1)) +(document[docNumber].widthOfTimeLineButton*(document[docNumber].numOfPatiants)); 
		...
		public void drawNamesButtons(/*Panel timeLinePanel*/)
		{
			...
			for (int panelCounter = 0 ;panelCounter<document[docNumber].numOfPanelsInSession;panelCounter++)
			{
				...
				for (int patiantCounter=0;patiantCounter<document[docNumber].numOfPatiants;patiantCounter++)
				{
					...
					sessionToolTip.SetToolTip(NameButtonArray[panelCounter,patiantCounter],"Additional information: \n" + this.document[docNumber].timeLine[patiantCounter].additionalInfoAboutPatiant);				
				}
			}
		}
		...
		private Point calculateButtonLocation(int patiantNumber, int buttonCounter)
		{
			...
			if (document[docNumber].timeLine[patiantNumber].MinutesArray[buttonCounter].levelOfInstrumentIntensity==0)
				level=2;
			return new System.Drawing.Point((StartLocation.X + ((buttonCounter-panelCounter*document[docNumber].numOfButtonsInTimeLine) * ButtonsSize.Width)), StartLocation.Y - level/*document[docNumber].timeLine[patiantNumber].MinutesArray[buttonCounter].levelOfInstrumentIntensity*//2);
		}
		...
		{	
			Size ButtonsSize=calculateButtonsSize(timeLinePanel[panelCounter]);
			Point StartLocation=calculateButtonsStartLocation(timeLinePanel[panelCounter],patiantNumber);
			return new System.Drawing.Point((StartLocation.X + (buttonCounter * ButtonsSize.Width))+((ButtonsSize.Width/2)-(document[docNumber].widthOfLable/2)), StartLocation.Y-(document[docNumber].heightOfLable/2) );
		}
		...
		private void InitializeComponent()
		{
			...
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(Sessions));
			...
			this.InstrumentImageList.ImageSize = new System.Drawing.Size(16, 16);
...
//				new Font("Arial", 80, FontStyle.Bold), Brushes.Black, 150, 125);
		...
		int counterr=0;
		private void Sessions_SizeChanged(object sender, System.EventArgs e)
		{
 ...
 */
		...
		private void Sessions_Paint(object sender, System.Windows.Forms.PaintEventArgs e)
		{
